...
module.exports = async () => {
  ...
  const rouletteJSON = JSON.parse(
    fs.readFileSync('./build/contracts/RouletteDev.json', 'utf8'),
  )
  const roulette = new web3.eth.Contract(rouletteJSON.abi, Roulette.address)
  ...
  const resolveRequestId = async (requestId) => {
    ...
    console.log(
      colors.green(
        `Resolved ${requestId.slice(0, 5)}...${requestId.slice(
          -5,
        )} with ${random}`,
      ),
    )
  }
  roulette.events.RequestedRandomness(async (error, data) =>
    resolveRequestId(data.returnValues.requestId),
  )

  console.log(colors.gray('Resolving queued requests...'))
  const previousRequests = await roulette.getPastEvents('BetRequest', {
    fromBlock: 0,
    toBlock: 'latest',
  })
