...
function betFor(betType, wallet) {
  return async function (value, result, amount = 1) {
    await rouletteInteractor.rollBets(
      wallet,
      [
        {
          betType,
...
contract('Roulette', async () => {
  describe('with liquidity', async () => {
    it('should add liquidity', async () => {
      await rouletteInteractor.setBetFee(0, {
    ...
    it('should work with big shares', async () => {
      ...
      await daiMockInteractor.burn(
        wallet1.address,
        await daiMockInteractor.balanceOf(wallet1.address),
      )
      await daiMockInteractor.burn(
        wallet2.address,
        await daiMockInteractor.balanceOf(wallet2.address),
      )
  ...
  describe('with failing requests', async () => {
    ...
    it('should not redeem if timelock has not passed', async () => {
      ...
      assert.equal(
        initialBalance - 1,
        await daiMockInteractor.balanceOf(wallet.address),
      )
  ...
  describe('with fees', async () => {
    ...
    it('should collect fee', async () => {
      const initialDeployerBalance = await daiMockInteractor.balanceOf(
        getDeployerWallet().address,
      )
      await rouletteInteractor.withdrawFees()
      assert.equal(await rouletteInteractor.getCollectedFees(), 0)
      assert.equal(
        await daiMockInteractor.balanceOf(getDeployerWallet().address),
        initialDeployerBalance + 0.04,
      )
    })
  })
})
