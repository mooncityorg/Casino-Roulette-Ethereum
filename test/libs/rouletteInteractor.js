...
async function getLastRequestId() {
  const previousRequests = await rouletteWeb3.getPastEvents('BetRequest', {
    fromBlock: 'latest',
    toBlock: 'latest',
  })
...
module.exports = {
  ...
  async getTotalLiquidity() {
    const roulette = await Roulette.deployed()
    return collapseTo18Decimals(
      (await roulette.getCurrentLiquidity()).toString(),
    )
  },
  async mintDAI(amount) {
    const roulette = await Roulette.deployed()
    await daiMockInteractor.mint(roulette.address, amount)
    return await roulette.forceAddLiquidity(
      expandTo18Decimals(amount).toString(),
      { from: getDeployerWallet().address },
    )
  },
  async burnDai(amount) {
    const roulette = await Roulette.deployed()
    return await roulette.forceRemoveLiquidity(
      expandTo18Decimals(amount).toString(),
      { from: getDeployerWallet().address },
    )
  },
  ...
  async setBetFee(amount) {
    const roulette = await Roulette.deployed()
    return await roulette.setBetFee(expandTo18Decimals(amount), {
      from: getDeployerWallet().address,
    })
  },
  async getMaxBet() {
