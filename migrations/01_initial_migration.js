  ...
  if (environment === 'ganache') {
    ...
    await deployer
      ...
      .then(async (daiToken) => {
        ...
        const vrfCoordinator = await deployer.deploy(
          VRFCoordinatorMock,
          linkToken.address,
        )
        ...
        if (process.env.INITIAL_LIQUIDITY) {
          const intialLiquidity = web3.utils.toWei(
            process.env.INITIAL_LIQUIDITY,
            'ether',
          )
        ...
        console.log(
          colors.cyan(
            'Copy paste these varaibles into your frontend .env file',
          ),
        )
